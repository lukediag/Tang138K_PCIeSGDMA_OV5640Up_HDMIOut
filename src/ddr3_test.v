module ddr3_test (
    ...
    output reg app_en,
    output reg [28-1:0] app_addr,
    input wr_data_rdy,
    output reg [128-1:0] app_wdf_data,
    output reg app_wdf_wren,
    output reg app_wdf_end,
    output [16-1:0] app_wdf_mask,
    output app_burst,
        ...
        case(state)
            ...
            STATE_WRITE: begin
                if(app_rdy & wr_data_rdy) begin
                    if(app_addr >= 2**26-8) begin
                        ...
                        app_cmd <= 0;
                        app_en <= 1;
                        app_wdf_data <= 128'hAAAA_AAAA_AAAA_AAAA_AAAA_AAAA_AAAA_AAAA;
                        app_wdf_wren <= 1;
                        app_wdf_end <= 1;
                        app_addr <= app_addr + 8;
            ...
            STATE_READ_CHECK: begin
                if(app_rd_data_valid) begin
                    if(app_rd_data == 128'hAAAA_AAAA_AAAA_AAAA_AAAA_AAAA_AAAA_AAAA) begin
                        num_ok <= num_ok + 1;
                    end
                    else begin
                        num_ok <= num_ok;
